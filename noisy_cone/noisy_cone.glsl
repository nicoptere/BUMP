...
const float MAX_TRACE_DIST = 10.0;	
...
{
    pos *= rotationMatrix( vec3(0.,0., 1. ), PI );
    float sphere1 = sdCone( pos - vec3( 0.,2.,0. ), vec3( 0.5,0.3,3.) ) ;
...
vec3 render(in vec3 ro, in vec3 rd, in vec2 uv)
{
    ...
    vec3 surfaceColor = .25*mix( .2*blue, orange, rd.y*3.9);
    ...
    if( t > -.50 ){
   		...
   		vec3 lightPos = normalize( vec3( 3.-abs( cos( u_time * 1.1 )),  -0.000,  - .5) );
    	...
    	float diffStrength = max( pow( dot(r, l), 2.5 ), 0.0 ) * .5 + max( pow( dot(n, l), 1. ), 0.0 ) * .5;
        float ambientStrength = clamp(0.05 + .5 * n.y, 0.0, 1.0);
        vec3 ambientColor = blue;

        ambientColor *= ambientStrength*.5;
